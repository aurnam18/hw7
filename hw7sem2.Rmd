...
diffprop( ~ LonR_fold | Sex, data = armfold ) #difference in proportions

#Part C
prop.test((LonR_fold == 1) ~ Sex, data=armfold)

# Part C: Compute standard error
std_error <- sqrt((left_male * (1 - left_male) / num_male) + 
                  (left_female * (1 - left_female) / num_female))
...
prop.test((voted1998 == 1) ~ GOTV_call, data=govt_matched) #95 percent confidence interval does not contain 0 so not statistically significant
```

Government calls may have increased the likelihood of voting in 1998, but since our confidence interval contains 0, we cannot be sure of this. There isn't sufficient evidence that government calls increased the likelihood of voting in 1998.
